...
for i in np.arange(400, 1000, 50, dtype=int):
    temparr = np.array([])
    tempwaits = np.array([])
    tempturns = np.array([])
    for j in range(5):
        ...
        responce = subprocess.run(['./hw1','4','4','100','30','1', str(i), str(random.randint(1,300)),'7'], stdout=subprocess.PIPE)
        end = time.time()
        ...
        currwait, currturn = responceAnalysis(responce.stdout.decode('utf-8'), 100, 1000*(start-end))
...
print("4 Consumers, 4 Producers, 100 Products, Queue Size 30 (quantum 10-510 inc 50): ", initialOptions)

print("Displaying Graph...")
plt.plot(np.arange(400, 1000, 50, dtype=int), waits, '-r^')
plt.xlabel("Quantum (fn(10))")
plt.ylabel("Time (ms)")
plt.title("Avg Wait Time vs Quantum")
plt.show()
plt.plot(np.arange(400, 1000, 50, dtype=int), turns, '-rs')
plt.title("Avg Turnaround Time vs Quantum")
plt.show()
plt.plot(np.arange(400, 1000, 50, dtype=int), initialOptions, '-ro',)
